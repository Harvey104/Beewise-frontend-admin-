...
export const satisticAction = (callback, err) => {
    let loginInfo = JSON.parse(localStorage.getItem('admin-auth'));
    var config = {
        method: 'get',
        url: SERVER_URL + 'admin/satistic',
        headers: {
            'x-access-token': loginInfo.accessToken
        }
    };
    ...
    .then(function (res) {
        console.log(res);
        res.data.message === 'success' ? callback(res.data.statistic) : err(res.data);
    })
    .catch(function (error) {
        console.log(error);
    });
...
export const getChallengeDetailList = (callback, err) => {
    let loginInfo = JSON.parse(localStorage.getItem('admin-auth'));
    var config = {
        method: 'get',
        url: SERVER_URL + 'admin/challenge_detail_list',
        headers: {
            'x-access-token': loginInfo.accessToken
        }
    };
