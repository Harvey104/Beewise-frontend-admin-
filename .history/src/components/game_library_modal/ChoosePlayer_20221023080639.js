...
const VerifyComponent = (props) => {
  ...
  const rowSelection = {
    onChange: (selectedRowKeys, selectedRows) => {
      ...
      setSelectedPlayer(selectedPlayer.push(selectedRows));
    },
          ...
          <Col span={14} style={{height: "100%"}}>
            {
              mode === "select" ?
                ...
                <div className="w-100 v-center">
                  <Button className="small-button hm" onClick={() => sdfsdfsdf{}}>Añadir seleccionados</Button>
                </div>
                </>)
              : 
                (<div>
                      ...
                      <List.Item key={item.id}>
                        <List.Item.Meta
                          title={<div className="v-around list-title">
                            {"# " + item.id + "  " + item.name + " "} 
                            <TiDeleteOutline onClick={() => removePlayer(item.id)} style={{color: "#EB5757", fontSize: "20px"}}/>
                          </div>}
                        />
                      </List.Item>
                    )}
