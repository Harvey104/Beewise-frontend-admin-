...
const AutoSetTeamComponent = (props) => {
  ...
  useEffect(() => {
    if (!state) {
      ...
      let playerCas = 0;
      if (teamCnt != 0) {
        for (let loop = 0; loop < selectedPlayer.length; loop+=teamCnt){
          for (let i = 0; i < teamCnt; i++) {
            ...
            //   playerCas++;
            // }
            teamArr[i].push(dividedPlayer[i * (loop + 1]);
          }
        }
        for(let loop = selectedPlayer.length-1; loop < selectedPlayer.length - selectedPlayer.length % teamCnt; loop--){
