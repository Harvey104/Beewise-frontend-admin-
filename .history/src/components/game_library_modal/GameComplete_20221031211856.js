...
const GameCompleteComponent = (props) => {
  ...
  const addCompetition = props.addCompetition;
  ...
  const listData = [
    ...
    },
    {
      ...
      title: (<>{"Inicia: " + startDate.toDateString()}<br/>{"Finaliza: " + termDate.toDateString()}</>)
    },
  ];
  return (
    <div className="flex-column v-between h-center" style={{width: "90%", height: "100%"}}>
      ...
      <div className="w-100 v-center">
        <Button className="small-button hm" onClick={() => addCompetition(GAME_MODAL_STEP.CREATE_SUCCESS)}>Añadir seleccionados</Button>
      </div>
    </div>
  );
};
