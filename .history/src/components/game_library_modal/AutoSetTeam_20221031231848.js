...
const AutoSetTeamComponent = (props) => {
  ...
  useEffect(() => {
    if (!state) {
      let teamArr = [];console.log(dividedPlayer);
      let playerCas = 0;
      if (teamCnt != 0) {
        for (let loop = 0; loop < (Math.ceil(selectedPlayer.length / teamCnt)); loop++){
          for (let i = 0; i < teamCnt; i++) {
            if (!teamArr[i]) { teamArr[i] = []; }
            if (dividedPlayer[playerCas] !== undefined) {
      ...
      if (teamArr.length != 0) {
        let teamData = [];
        let teamTableList = teamArr.map((team, index)=> {console.log(team);
          let members = [];
          let teammates = team.map((member, index) => {console.log(member);
