...
import { addCompetitionAction, getCompetitionList } from '../../components/actions/Game';
...
export default function GameLibraryPage() {
  ...
  useEffect(() => {
    if (!mountState) {
      ...
      getCompetitionList((list) => {
        ...
        let active = [];
        let finished = [];
        list.map((compete) => {console.log(compete.endDate);
            if (new Date(compete.startDate) > new Date()) {
                pending.push({
                  competation: compete.competitionName,
                  play: compete.topic,
                  category: compete.topic,
                  player: compete.player_cnt,
                  shared: "08-04"
                });
            }
  ...
  const addCompetition = () => {
    let teammates = teams.map((team) => {
      let members = team.map((mem) => {
        return mem.key;
      });
      return {
        weight: 1,
    ...
    addCompetitionAction({
      ...
      console.log(data);
      setModalStep(GAME_MODAL_STEP.CREATE_SUCCESS);
    });
  }
    ...
    <div className='dashboard'>
              ...
              <Carousel>
                <Carousel.Item>
                  <Row>
                      ...
                      {
                        topicList.map((topic) => {
                          return <Col span={4} className="padding-small">
                            <div className='library-card flex-column h-center padding-small v-between gap-medium'>
                                <Text className='sub-title'>{topic.topic}</Text>
                                <Text className='sub-text'><img src='/img/icon/answer.png'/>22 preguntas</Text>
                                <Button className={`long-button hs back-yellow`} onClick={() => {showModal(); setTopicId(topic.id);}} type="primary">
                                  Iniciar
                                </Button>
                            </div>
                          </Col>
        ...
        >
            ...
            >
              <Table
                  ...
                  dataSource={competitionList.pending}
                  pagination={false}
                  rowSelection={{
                    type: 'checkbox',
                    ...rowSelection,
            ...
            >
              <Table
                  columns={competationColumn}
                  dataSource={competitionList.active}
                  pagination={false}
                  rowSelection={{
                    type: 'checkbox',
                    ...rowSelection,
            ...
            >
              <Table
                  columns={competationColumn}
                  dataSource={competitionList.finished}
                  pagination={false}
                  rowSelection={{
                    type: 'checkbox',
                    ...rowSelection,
        ...
        >
            ...
            >
                        ...
                        <Text className='medium-title color-black' style={{marginLeft: "66px", fontSize: "18px"}}>
                          {
                            ...
                            modalStep === GAME_MODAL_STEP.SET_TEAM ? "Nombre Competencia / Código 1-1-1-1-1" :
                            modalStep === GAME_MODAL_STEP.CHOOSE_DATE ? "Nombre Competencia / Código 1-1-1-1-1" :
                ...
                {
                  modalStep === GAME_MODAL_STEP.VERIFY ? <Verify previous={handleCancel} next={setModalStep} setCompetitionName={setCompetitionName}/> :
                  ...
                  modalStep === GAME_MODAL_STEP.GAME_COMPLETE ? <GameComplete next={setModalStep} selectedPlayer={selectedPlayer} teamCnt={teamCnt} startDate={startDate} termDate={termDate} addCompetition={addCompetition}/> : 
