...
import { teamRankingAction, getChallengeDetailList } from '../../components/actions/Game';
      ...
      teamRankingAction((data) => {
        ...
        let teamRankData = data.map((item, index) => {
          ...
          return {
            id: index + 1,
            position: '#' + (index + 1),
            equi: item.competition.competitionName + " " + item.teamName,
            score: item.score,
          }
        });
        setWinTeams(winners);
                    ...
                    return <div className='v-between mt-small'>
                      ...
                      style={{
                        ...
                        <div className='flex-column v-center'>
                          <Text style={{ fontFamily: 'Poppins', fontWeight: 600, fontSize: "16px", color: "#6A67CE", }}>{player.name}</Text>
                          <Text style={{ fontFamily: 'Poppins', fontWeight: 600, fontSize: "14px", color: "#6A67CE", }}>{player.equi}</Text>
                        </div>
                        <Text style={{ fontFamily: 'Poppins', fontWeight: 500, fontSize: "14px", color: "#6A67CE", }}>{player.score} pts</Text>
                      </div>
                    </div>
                  })
                }
                <div className='competition-panel mt-small'>
                  <List
                    ...
                    renderItem={item => (
                      <List.Item style={{padding: "none"}}>
                        <Row style={{width: "100%"}}>
                          <Col span={4}><Text className="medium-title color-b">{"# " + item.id}</Text></Col>
                          <Col span={7}><Text className="medium-title color-b">{item.equi}</Text></Col>
                          <Col span={7}><Text className="medium-title color-b">{item.name}</Text></Col>
                          <Col span={6}><Text className="medium-title color-b">{item.score}</Text></Col>
                        </Row>
                      </List.Item>
                    )}
                ...
                <div className='mt-small' style={{overflowY: "auto", height: "654px"}}>
                  {
                    challengeList.map((item) => {
                      return <div className='mt-small'>
                      <div className='padding-small flex-column' style={{background: "#FFFAEC", borderRadius: "10px"}}>
                        <Text style={{fontFamily: 'Inter', fontWeight: 600, fontSize: "16px", lineHeight: "24px", color: "#6A67CE"}}>{item.player1Name} vs {item.player2Name}</Text>
                        <Text className='mt-small' style={{fontFamily: 'Inter', fontWeight: 500, fontSize: "14px", lineHeight: "18px", color: "#6A67CE"}}>{item.player1Team} desafío a {item.player2Team}</Text>
                        <Text style={{fontFamily: 'Inter', fontWeight: 500, fontSize: "14px", lineHeight: "18px", color: "#6A67CE"}}>Ganador: {item.player1Team}</Text>
                      </div>
                    </div>
                    })
                  }
