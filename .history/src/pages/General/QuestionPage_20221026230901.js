  ...
  const questionTableColumn = [
    {
      ...
      render: (quiz, record) => (
        editId === record.id ?
          <Input placeholder="Ingrese nombre del juego" className='hm' value={quiz}/>
          :
          quiz
      )
    },
        ...
        editId !== record.id ?
          ...
          (
            ...
            {alternative.map((alter, index) => (  
              <div className='h-center mt-bit'>
                {alter.state === true ? <CheckCircleFilled style={{color: "#01cf96",fontSize: '20px'}} /> : <CloseCircleFilled style={{color: "#f42b35",fontSize: '20px'}} />}
                <Input placeholder="Ingrese nombre del juego" className='hm' value={alter.title}/>
              </div>
            ))}  
          </div>)
          )
    ...
    },
    {
      ...
      render: (category, record) => (
        editId === record.id ?
          <Input placeholder="Ingrese nombre del juego" className='hm' value={category}/>
          :
          category
      )
    },
    {
      ...
      render: (time, record) => (
        editId === record.id ?
          <Input placeholder="Ingrese nombre del juego" className='hm' value={time}/>
          :
          time
      )
    },
    {
      ...
      render: (_, record) => (
        <div className='v-around'>
          ...
          <img src='/img/icon/delete.png' onClick={() => setEditId(record.id)}/>
        </div>
      )
    },
  ];
