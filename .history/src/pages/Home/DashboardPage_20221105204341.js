...
import { teamRankingAction, getCompetitionList, satisticAction } from '../../components/actions/Game';
...
export default function DashboardPage() {
  ...
  const [categoryList, setQuizList] = useState([]);
  ...
  useEffect(() => {
    if (!mountState) {
      teamRankingAction((data) => {
        let teamRankData = data.map((item, index) => {
          return {
            ...
            member: item.persons,
            score: item.score,
            hit: ((item.score/(item.persons * 100)) * 100) + "%",
            challengeC: 2,
            challengeP: 1,
          }
        });
      ...
      getCompetitionList((list) => {
        ...
        setPendingCnt(pendingCnt);
        setStartCnt(startCnt);
        setCompleteCnt(completeCnt);
      });

      satisticAction((data) => {
        setQuizPer(data.quizSta);
        let totalCnt = 0;
        data.categoryCnt.map((category) => {
          totalCnt += parseInt(category.category_cnt);
        });
        let categoryList = data.categoryCnt.map((category, index) => {
          return {
            key: index + 1,
            category: category.category,
            total: totalCnt,
            hit: category.category_cnt,
            manage: Math.round((category.category_cnt/totalCnt)*100),
          }
        });
        setCategoryList(categoryList);

        let quizList = data.quizCorrect.map((quiz, index) => {
          return {
            key: index + 1,
            ask: quiz.question,
            correct: `${Math.round((quiz.correntCnt/quiz.testCnt)*100)}% (${quiz.correntCnt})`,
            incorrect: `${Math.round(((quiz.testCnt-quiz.correntCnt)/quiz.testCnt)*100)}% (${quiz.testCnt-quiz.correntCnt})`,
            total: quiz.testCnt,
          };
        });
