  ...
  useEffect(() => {
    if (!mountState) {
      ...
      satisticAction((data) => {
        ...
        let quizs = data.quizCorrect.map((quiz, index) => {
          if (quiz.correctCnt) {
            return {
              key: index + 1,
              ask: quiz.question,
              correct: `${Math.round((quiz.correctCnt/quiz.testCnt)*100)}% (${quiz.correctCnt})`,
              incorrect: `${Math.round(((quiz.testCnt-quiz.correctCnt)/quiz.testCnt)*100)}% (${quiz.testCnt-quiz.correctCnt})`,
              total: quiz.testCnt,
            };
          } else {
            return {
